...
const PlayerTable = ({ players, setPlayers }) => {
    ...
    try {
        ...
        if (!res.ok) {
            const errorText = await res.text(); // helpful for logging/debugging
            throw new Error(`Unauthorized (${res.status}): ${errorText}`);
        }

        const player = await res.json(); // 👈 only parse JSON after status check
        setFailedAttempts(0);
        navigate(`/edit/${id}`, { state: { password: input } }); // 👈 only on success
